...
| `G3-01` | Maintain this sprint brief and one evidence ledger; do not expand the gate through implementation convenience. | All `G3-00`–`G3-21` rows have explicit scope, verification, and disposition. | Candidate-local passed; hosted binding open |
...
| `G3-16` | Add local hooks and hosted CI using the same contributor commands and locked installs. Hooks must not hide CI-only behavior. | Hook configuration is executable; CI covers supported toolchains, caches only locked inputs, uploads no private data, and records a green required run. | Local hook execution passed; hosted run open |
